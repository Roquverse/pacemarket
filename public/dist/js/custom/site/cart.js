...
$(document).on('click', '.add-to-cart', function() {
    ...
    let dupOptionBox = [];
    let cnt = 0;
    itemType = $(this).attr('data-type') ?? itemType;
...
function shippingTax(tax = 0, shippingArr = null, totalPrice, shippingIndex = 0, displayTax = null)
{
                            ...
                            <div class="flex justify-between mt-3">
                                <div class="text-sm">${i}</div>
                                    <div class='text-sm text-right'>
                                        <span class="customTax">${decimalNumberFormatWithMultiCurrency(v)}</span>
                                    </div>
                            </div>
                            `;
        });
        ...
        customTax += `
                <div class="flex justify-between mt-3">
                    <div class="text-sm">${jsLang('Tax')}</div>
                        <div class='text-sm text-right'>
                            <span class="customTax">${decimalNumberFormatWithMultiCurrency(tax)}</span>
                        </div>
                </div>
                `;
    }
                       ...
                       <li>
                            <div class="mt-2 w-full">
                                <div class="flex justify-between items-center w-full shipping-radio-button-custom">
                                    ...
                                    <label> ${decimalNumberFormatWithMultiCurrency(vv)}</label>
                                </div>
                            </div>
                       </li>
                  `;
    ...
    $('#cart-total').text(decimalNumberFormatWithMultiCurrency((totalPrice - couponOffer) + totalTax + parseFloat(shipingIntailValue)));
    $('#cart-total-d').text(decimalNumberFormatWithMultiCurrency((totalPrice - couponOffer) + totalTax + parseFloat(shipingIntailValue)));
    
    if ($('.main-body .page-wrapper').find('#order-checkout-container').length) {
        $('#equivalent_price').text(decimalNumberFormatWithEquiCurrency((totalPrice - couponOffer) + totalTax + parseFloat(shipingIntailValue)));
    }
}
        ...
        $.each(carts, function (index, value) {
            ...
            <div class="flex cursor-pointer border-gray-100 cart-item-header mt-5" id="cart-item-header-${index}">
                ...
                <div class="flex flex-col justify-center text-sm w-64 ml-5 rtl-mr-20">
                    ...
                    ${optionHtml}
                    <div class="cart-item-quantity roboto-medium font-medium text-gray-10 text-base leading-5">${value['quantity']} × ${getDecimalNumberFormatMultiCurrency(value['price'])}</div>
             ...
             <div class="absolute justify-center bg-white flex flex-col inset-x-0 px-30p mt-30p bottom-5">
                    <div class="border-t border-gray-2">
                        <div class="pt-4 pb-30p flex justify-between dm-sans font-medium text-gray-12 text-22">
                            ...
                            <p id="cart-item-total-price">${decimalNumberFormatWithMultiCurrency(totalPrice)}</p>
                       </div>
                    </div>
...
function quantityPriceUpdate(qty, price, totalPrice, discountAmount, discountType, actualPrice)
{
    ...
    discountType != "Percent" ? $('#discount-amount-'+cartIndex).text(getDecimalNumberFormatMultiCurrency(discountAmount * qty)) : '';

    $('#cart-item-header-'+cartIndex+' .cart-item-quantity').text(qty+ " × "+getDecimalNumberFormatMultiCurrency(price));
    $('#cart-item-header-'+cartIndex+' .cart-item-quantity-header').text(qty);
    $('#cart-item-header-'+cartIndex+' .cart-item-price').text(getDecimalNumberFormatMultiCurrency(price * qty));
    totalPriceUpdate(totalPrice);
    checkingCheckbox();
}
...
{
    $('#cart-item-total-price').text(decimalNumberFormatWithMultiCurrency(totalPrice));
}

function totalPriceByChecked(totalPrice, totalTax = 0, totalShipping = 0)
{
    couponOffer = 0;
    $('#couponOffer').text(decimalNumberFormatWithMultiCurrency(couponOffer));
    $('#cart-subtotal').text(decimalNumberFormatWithMultiCurrency(totalPrice));
                ...
                if (data.status == 1) {
                    ...
                    $.each(data.data, function (i,v) {
                        ...
                        <div class="flex justify-between mt-15p delete-coupons-${jsLang(v.id)}">
                                    ...
                                    <p class="text-gray-12 dm-sans font-medium text-sm coupon-amount">- ${decimalNumberFormatWithMultiCurrency(v.calculated_discount)}</p>
                                </div>
                        `;
