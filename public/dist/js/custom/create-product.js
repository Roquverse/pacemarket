...
$(function () {
    ...
    const saveAttributeHandler = (params, response) => {
        ...
        if (response.status.code !== 200) {
            ...
            loadAttributeOptions({ blockable: params.blockable });
            ajaxNotify(response.status);
        }
    };
    ...
    const loadAttributeOptions = (params = {}) => {

        $.makeAjaxCall(
            itemUrl,
            {
                action: "load_attribute_options",
            },
            {
                type: "post",
                success: handleLoadAttributeOptionAction,
                callbackParam: params,
            }
        )
    };

    const handleLoadAttributeOptionAction = (params, response) => {
        $(".attribute-option-item-lists").html(response.records.html);
        ...
        makeAjaxCall: function (_url, _data = {}, _options = {}) {
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    defaults.failed(defaults.callbackParam, xhr?.responseJSON?.response);
                },
            });
        },
    });
    ...
    const getFilePreview = (file, name) => {
        if(videoExtensions.map((extension) => file.url.includes(extension)).includes(true)){
            var src = `<video class="fit-boxed">
                        <source src="${file.url}" type="video/mp4">
                       </video>`
        }else{
            var src = `<img class="fit-boxed" src="${file.url}" alt="${file.name}">`
        }
