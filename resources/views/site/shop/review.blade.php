    @php
        if (isset($shop) && $shop->vendor_id) {
             $vendor = \App\Models\Vendor::with('reviews')->where('id', $shop->vendor_id)->first();
        } else {
            $vendor = \App\Models\Vendor::with('reviews')->where('id', request()->vendor_id)->first();
        }
        ...
        <div class="container">
            <div class="pt-8 flex">
                <div class="flex-shrink-0 flex flex-col">
                    <img class="w-14 h-14 ltr:mr-4 rtl:ml-4 inline-flex items-center justify-center rounded-full bg-indigo-100 flex-shrink-0" src="{{ isset($review->user) ? optional($review->user)->fileUrl() : asset('public/dist/img/default-image.png') }}" alt="{{ __('Coustomer image') }}">
                </div>
