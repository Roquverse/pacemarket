...
@section('content')
    @php
        ...
        $vendors = App\Models\Vendor::getAll()->where('status', 'Active')->pluck('name', 'id');

        if (isActive('Shop')) {
            $shops = \Modules\Shop\Http\Models\Shop::getAll()->where('status', 'Active')->pluck('name', 'id');
        }
        ...
        foreach ($cartsValue as $key => $val) {
            ...
            $product = App\Models\Product::select('id', 'available_from', 'available_to', 'status', 'manage_stocks', 'total_stocks', 'shop_id', 'sale_to', 'sale_price', 'regular_price', 'type', 'parent_id', 'vendor_id')->where('id', $val['id'])->with('parentDetail:id,vendor_id')->first();
            ...
            if ($product->manage_stocks == 1 && $product->meta_backorder == 0) {
                $inventoryEnable = $val['quantity'] <= $product->total_stocks;
            }
        ...
        $cartData = collect($cartWithIndex);
    ...
    <section class="layout-wrapper px-4 xl:px-0">
        ...
        @include('site.layouts.includes.order-steps', ['stepsNumber' => 1])
    </section>

    <section class="text-gray-600 body-font relative layout-wrapper px-4 xl:px-0 mt-11 md:mt-80p"
                                                    ...
                                                    <div class="col-span-2 mt-4">
                                                        <div
                                                            ...
                                                            <h4 class="text-sm font-medium dm-sans text-gray-12">
                                                                <span></span><span
                                                                    class="cart-item-price">{{ multiCurrencyFormatNumber($cart['price']) }}</span>
                                                            </h4>
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <div class="w-full px-5 py-5 text-gray-12 font-medium dm-sans">
                            ...
                            @if (isActive('Coupon') && preference('coupons') == 1)
                                <div class="flex justify-between mt-3 display-none couponOffer">
                                    ...
                                    <div class='dm-sans text-sm text-gray-12 font-medium'><span
                                            id="couponOffer">{{ formatMultiCurrencyAmount($couponOffer) }}</span></div>
                                </div>
                            @endif
                            ...
                            <div class="flex justify-between mt-3 border-t">
                                ...
                                <div class="text-base mt-2">
                                    <span id="cart-total">{{ formatMultiCurrencyAmount($totalPrice - $couponOffer) }}</span>
                                </div>
                            </div>
                            @if (isActive('Coupon') && preference('coupons') == 1)
                                <div class="mt-10">
                            ...
                            @if (is_array($gateways) && count($gateways) > 0)
                                <div>
                                    ...
                                    <div class="flex flex-wrap gap-x-6 gap-y-2 mt-5 mb-5">
                                        ...
                                        <div>
                                            <img class="w-16 h-7 m-auto object-contain" src="{{ asset(moduleConfig($gateway->alias . '.logo')) }}" alt="{{ __('Image') }}">
                                        </div>
                                        @endforeach
                                    </div>
                                </div>
