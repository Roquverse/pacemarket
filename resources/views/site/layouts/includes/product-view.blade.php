...
<div id="item-view-load">
                                ...
                                <div class="swiper border rounded-md w-full swiper-container-mainV">
                                    <div class="swiper-wrapper" id="zoomImageV">
                                        @foreach ($images as $ProductImage)
                                            ...
                                            @endphp
                                            <div class="swiper-slide minimum-height {{ $isVideo ? '' : 'zoom' }}" style="background-image: url({{ getBackgroundImage($ProductImage) }});">
                                                {!! $src !!}
                                            </div>
                                        @endforeach
                                                    ...
                                                    @php
                                                        $isvideo = false;
                                                            if(in_array(pathinfo($ProductImage, PATHINFO_EXTENSION), getFileExtensions(6))) {
                                                                ...
                                                                $isVideo = false;
                                                                $src = '
                                                                <img class="p-1.5 object-cover h-12 cursor-pointer " src="' . $ProductImage . '" alt="Image...">
                                                                ';
                                                            }
                                                    @endphp
                                                    {!! $src !!}
                            ...
                            <div>
                                ...
                                <div class="flex md:flex-col flex-wrap lg:flex-row justify-start items-center md:items-start lg:justify-start lg:items-center gap-x-4 md:gap-x-0 lg:gap-x-4 md:mt-3 mt-3">
                                    ...
                                    <div class="flex justify-start items-center x:gap-x-3 gap-x-4 mt-0 md:mt-4 lg:mt-0">
                                        {{-- share button --}}
                                        @include('site.layouts.section.product-details.share-button')
                                ...
                                <div class="flex">
                                    @if ($product->isVariableProduct())
                                        ...
                                        @endphp
                                        <span class="text-2.5xl dm-bold text-gray-12" id="varMinMaxPriceV">{{ multiCurrencyFormatNumber($filterVariation['min']) }}
                                            - {{ multiCurrencyFormatNumber($filterVariation['max']) }}</span>
                                    @endif
                                </div>
                                <div class="flex">
                                    @if ($offerFlag)
                                        <p class="dm-bold">
                                            <span
                                                ...
                                                id="item_priceV">{{ !$product->isVariableProduct() ? multiCurrencyFormatNumber($product->priceWithTax($displayPrice, 'sale', false)) : multiCurrencyFormatNumber($sale_price) }}</span>
                                            <span class="text-lg line-through text-gray-10 ltr:pl-1 rtl:pr-1 mt-0.5 {{ $product->isVariableProduct() ? 'display-none' : '' }}">{{ $type != 'Variable Product' ? multiCurrencyFormatNumber($product->priceWithTax($displayPrice, 'regular', false)) : multiCurrencyFormatNumber($regular_price) }}</span>
                                        </p>
                                    @else
                                        <p class="dm-bold">
                                                ...
                                                <span class="color-options">
                                                    @if($attributes[$key]['additional_value'])
                                                        @foreach($attributes[$key]['additional_value'] as $cdk=>$value)
                                                            ...
                                                            @endphp
                                                            <input type="radio" name="option[{{$key}}]" value="{{ $attributes[$key]['value'][$cdk] }}" {{isset($default_attributes[$key]) && $default_attributes[$key] == $backValue ? 'checked' : ''}}  class="custombox-selected cursor-pointer item-variationsV {{ $attributes[$key]['attribute_shape'] == 'square' ? 'square' : ''}}" data-id="{{ $attributes[$key]['value'][$cdk] }}" data-position="{{ $position }}" style="background: {{ $value }}">
                                                        @endforeach
                                                    @elseif($attributes[$key]['attribute_id'])
                                                        ...
                                                        @endphp
                                                        <input type="radio" name="option[{{$key}}]" value="{{ $details->value }}" data-id="{{ $details->id }}" data-position="{{ $position }}"  {{isset($default_attributes[$key]) && $default_attributes[$key] == $backValue ? 'checked' : ''}}  class="custombox-selected cursor-pointer item-variationsV" style="background: {{ $details->additional_values }}">
                                                    @endforeach       
                                                    @endif
                                                </span>
                                                @else
                            ...
                            <div
                                ...
                                <div class="mt-3 md:mt-0 w-full ltr:md:ml-4 rtl:md:mr-4">
                                    @if ($product->isExternalProduct())
                                        ...
                                        @endif
                                    @else
                                        @doAction('before_quick_view_add_to_cart', $definedData)
                                        ...
                                        @doAction('after_quick_view_add_to_cart', $definedData)
                                    @endif
                                </div>
                            </div>
                        </div>
    ...
    <script src="{{ asset('public/dist/js/custom/site/product-view.min.js?v=2.8') }}"></script>
