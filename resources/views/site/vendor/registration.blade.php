    ...
    <div class="3xl:mx-470p 2xl:mx-64 xl:mx-32 md:mx-20 mx-5 mb-70p 3xl:-mt-132p -mt-28 relative border border-gray-2 bg-white rounded-lg">
        ...
        <form method="post" action="{{ auth()->user() ? route('seller.store.request') : route('site.seller.signUpStore') }}"
            ...
            <div class="grid lg:grid-cols-2 grid-cols-1 lg:gap-3">
                ...
                @else
                    <div class="lg:order-none lg:mb-3 mb-3.5">
                        <label class="text-sm dm-sans font-medium capitalize text-gray-12 require-profile">{{ __('Alias') }}</label>
                        <input class="border-gray-2 mt-1.5 lg:mt-1p rounded-sm w-full h-46p pl-18p roboto-medium font-medium text-sm text-gray-10 form-control focus:border-gray-12" type="text" name="alias" required oninvalid="this.setCustomValidity('This field is required.')" value="{{ old('alias') }}" placeholder="{{ __('Enter Your :x', ['x' => __('Alias')]) }}">
                        @error('alias')
                            <span class="password-validation-error block text-11 mt-1 text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
