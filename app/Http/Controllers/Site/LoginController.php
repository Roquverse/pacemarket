...
class LoginController extends Controller
{
    ...
    public function signUp(Request $request)
    {
        ...
        if ($response['status'] == 1) {
            return redirect()->route('login')->withSuccess(__('Registration successful. Please login to your account.'));
        }
    ...
    public function validMail(Request $request, $contactInfo)
    {
        ...
        if (isRecaptchaActive()) {
            $request['gCaptcha'] = $request['captcha'] ?? null;
            $captchaRule = 'required|captcha';

            $validation = \Validator::make($request->all(), [
                'gCaptcha' => $captchaRule ?? 'nullable',
            ]);

            if ($validation->fails()) {
                return ['status' => 'fail', 'message' => $validation->errors()->first()];
            }
        }
