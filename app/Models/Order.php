...
class Order extends Model
{
    ...
    {
        $query = $this->hasOne('Modules\Gateway\Entities\PaymentLog', 'code', 'id');
    ...
    {
        return $this->hasOne(OrderShippingTrack::class)->where('track_type', 'order');
    }
     ...
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function shippingTracks($trackType = 1)
    {
        $query = $this->hasMany(OrderShippingTrack::class);

        $query->where('track_type', $trackType ? 'product' : 'order');
