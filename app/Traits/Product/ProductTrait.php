...
trait ProductTrait
{
    ...
    public function isStockManageable()
    {
        if (! isset(static::$isStockManageableCache[$this->id])) {
            static::$isStockManageableCache[$this->id] = (preference('manage_stock') && $this->manage_stocks) || $this->hasVariationWithEnabledStock() ? 1 : 0;
        }

        return static::$isStockManageableCache[$this->id];
    }
    ...
    public function getStockStatus()
    {
        if (! $this->isStockManageable()) {
            return empty($this->meta_stock_status) ? 'In Stock' : $this->meta_stock_status;
        }

        if ($this->total_stocks > 0 || $this->getVariationStockQuantity() > 0 || $this->hasVariationWithoutEnabledStock()) {
            return 'In Stock';
        }

        return $this->meta_backorder == 1 ? 'On Backorder' : 'Out of stock';
    }
        ...
        foreach ($variations as $variation) {
            if ($variation->status == ProductStatus::$Published && isset($variation->attributes)) {
                ...
                $attributePriceWithId[] = [
                    ...
                    'manage_stocks' => $isStockManageable == 0 ? $isParentStockManageable : $isStockManageable,
                    'stock_quantity' => $isStockManageable == 0 ? $getParentStockQuantity : $variation->getStockQuantity(),
                    'isOutOfStock' => $isStockManageable == 0 ? $isParentOutOfStock : $variation->isOutOfStock(),
                    'lowStockThreshold' => $isStockManageable == 0 ? $getParentStockThreshold : $variation->getStockThreshold(),
                    'backOrders' => $isStockManageable == 0 ? $parentMetaBackOrder : $variation->meta_backorder,
